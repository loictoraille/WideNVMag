...
uicontrol('Parent',ffp_panel,'Style','edit','tag','FitMethod','FontSize',12,'units','normalized','Position',[0.04 0.80 0.15 0.2],'String',num2str(FitParameters.FitMethod),'Tooltip',['5 = Standard fit method with possible baseline removal, single lorentzian per peak' 10 '6 = Same fit method as 5 but the field calculation uses a different equation, adapted to randomly oriented ensembles of NV centers' 10 '7 = Fast barycenter method that only looks for the middle frequency' 10 '8 = Fast correlation method that only looks for the middle frequency (better than barycenter)'],'Callback',@UpdateFit);
