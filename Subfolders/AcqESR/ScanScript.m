...
tic
startTime = datetime('now'); % Capture l'heure de depart
    ...
    for ii=1:NPoints % Starting the RF loop
        ...
        if ~TestWithoutHardware
            if RANDOM == 1
                ...
                pause(0.05/NPerm); % plus il y a de permutations, moins la pause a besoin d'etre longue
            else
                if mod(Acc,2) == 1 % montant puis descendant une acq sur deux pour eviter l'erreur due au retour au premier point de la rampe
